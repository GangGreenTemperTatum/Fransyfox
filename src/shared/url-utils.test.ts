...
describe('shared/url-utils', () => {
  test('cleans URL by preserving query params but removing fragment', () => {
    const utils = (globalThis as { FransyfoxUrlUtils?: { cleanUrl: (url: string) => string } }).FransyfoxUrlUtils;
  ...
  test('extracts JS URL from stack trace', () => {
    const utils = (globalThis as {
      FransyfoxUrlUtils?: { extractJsUrlFromStack: (stack?: string, fullstack?: string[]) => string | null };
    }).FransyfoxUrlUtils;
